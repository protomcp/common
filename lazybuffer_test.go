...
func (tc lazyBufferTest) test(t *testing.T) {
	buf := tc.setup()
	tc.action(buf)
	testutils.AssertEqual(t, tc.expected, buf.String(), tc.name)
}
...
func (tc lazyBufferChainTest) test(t *testing.T) {
	var buf LazyBuffer
	result := tc.chain(&buf)
	testutils.AssertEqual(t, tc.expected, result, tc.name)
}
...
func testLazyBufferNil(t *testing.T) {
	tests := []lazyBufferNilTest{
		newLazyBufferNilTest(
			...
			func(t *testing.T, result any) {
				testutils.AssertNil(t, result, "WriteString on nil buffer")
			},
		),
		newLazyBufferNilTest(
			...
			func(t *testing.T, result any) {
				testutils.AssertNil(t, result, "WriteRunes on nil buffer")
			},
		),
		newLazyBufferNilTest(
			...
			func(t *testing.T, result any) {
				testutils.AssertNil(t, result, "Printf on nil buffer")
			},
		),
		newLazyBufferNilTest(
			...
			func(t *testing.T, result any) {
				testutils.AssertEqual(t, "", result, "String on nil buffer")
			},
		),
		newLazyBufferNilTest(
			"nil Len returns 0",
			...
			func(t *testing.T, result any) {
				testutils.AssertEqual(t, 0, result, "Len on nil buffer")
			},
		),
	}
	...
	t.Run("nil Reset no panic", func(t *testing.T) {
		...
		testutils.AssertNoError(t, err, "Reset on nil buffer")
	})
}
...
func testLazyBufferLen(t *testing.T) {
	...
	testutils.AssertEqual(t, 0, buf.Len(), "initial length")

	buf.WriteString("Hello")
	testutils.AssertEqual(t, 5, buf.Len(), "length after Hello")

	buf.WriteRunes(' ', '世', '界')
	// "Hello 世界" - 5 + 1 + 3 + 3 = 12 bytes (世 and 界 are 3 bytes each in UTF-8)
	testutils.AssertEqual(t, 12, buf.Len(), "length after Unicode")

	buf.Printf(" %d", 42)
	testutils.AssertEqual(t, 15, buf.Len(), "length after Printf")
}
...
func testLazyBufferReset(t *testing.T) {
	...
	testutils.AssertEqual(t, "Hello world", buf.String(), "before reset")
	testutils.AssertEqual(t, 11, buf.Len(), "length before reset")

	buf.Reset()
	testutils.AssertEqual(t, "", buf.String(), "after reset")
	testutils.AssertEqual(t, 0, buf.Len(), "length after reset")

	// Should be able to use after reset
	buf.WriteString("New content")
	testutils.AssertEqual(t, "New content", buf.String(), "after reset and write")
}
...
func testLazyBufferLargeContent(t *testing.T) {
	...
	testutils.AssertEqual(t, iterations, lines, "number of lines")

	// Verify it starts and ends correctly
	testutils.AssertTrue(t, strings.HasPrefix(result, "Line 0:"), "starts with Line 0")
	testutils.AssertTrue(t, strings.Contains(result, "Line 999:"), "contains Line 999")
}
